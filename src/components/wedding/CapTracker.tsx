...
const CapTracker = () => {
  const [orders, setOrders] = useState<CapOrder[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<CapOrder | null>(null);
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    const { data, error } = await (supabase.from("cap_orders" as any) as any)
      .select("*")
      .order("created_at", { ascending: false });
    if (error) {
      toast({
        title: "Failed to load cap orders",
        description: error.message,
        variant: "destructive",
      });
    }
    if (data) setOrders(data as CapOrder[]);
    setLoading(false);
  }, [toast]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setOrders((prev) =>
          prev.map((o) => (o.id === editingId ? (data as CapOrder) : o))
        );
        ...
        const { data, error } = await (supabase.from("cap_orders" as any) as any)
          ...
          .single();
        if (error) throw error;
        setOrders((prev) => [data as CapOrder, ...prev]);
  ...
  const toggleField = async (
    ...
    const { error } = await (supabase.from("cap_orders" as any) as any)
      .update({ [field]: next })
      .eq("id", order.id);
    if (error) {
      // revert on failure
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `cap-orders-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                  {orders.length === 0
                    ? "No cap orders yet. Click \"Add Order\" to start tracking."
